...
func (c *CurrencyDay) GetData(CurrencyDate time.Time) {
	...
	for {
		if currDay == nil {
			...
			if currDay != nil { // GO'DA GATA KONTROLÜ BU ŞEKİLDE YAPILIYOR
				break
			}
		} else {
			break
...
func (c *tarih_Date) getData(CurrencyDate time.Time, XDate time.Time) *CurrencyDay {
	...
	if err != nil {
		...
		if response.StatusCode != http.StatusNotFound {
			...
			fmt.Println(currDay)
			SaveJSON("currencies.json", currDay) // json dosyasına bigiliyi bas.
			// fmt.Println(currDay.Currencies[0].CurrencyNameTR + " / " + strconv.FormatFloat(currDay.Currencies[0].BanknoteSelling, 'E', -1, 64)) // İSTEDİĞİN ALANI LİSTELE
		} else {
			currDay = nil
		}
	}
